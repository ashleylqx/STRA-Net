...
if model_no == 0:
    from custom_loss import kl_divergence, correlation_coefficient, nss, sim
elif model_no == 1:
    from custom_loss_td import kl_divergence, correlation_coefficient, nss, sim
...
from custom_generator import generator_td_simple_mopt, generator_td_prior_mopt_ds
from custom_generator import get_test_td_simple_mopt, get_test_td_prior_mopt
...
def Feature_dcross_res_matt_res_ds_masks(input_tensors=None, a_train=True, m_train=True):
    ...
    # Create model
    if FeatureNet_mode == 'feature':
        model = Model(input_tensors,
                      Concatenate()([A_conv_5_out_M2, sd_3, sd_4, sd_5, m3, m4, m5]))
        if phase == 'train':
            model.load_weights(model_path + "vap_model/U_td3_res_ds.h5",
                               by_name=True)
        elif phase == 'vis':
            pass

    elif FeatureNet_mode == 'main':
        if phase == 'train':
            pred = Conv2D(1, (1, 1), padding='same', activation='sigmoid')(A_conv_5_out_M2)
            model = Model(input_tensors, [pred, pred, pred, pred,
                                          sd_3, sd_3, sd_3, sd_3,
                                          sd_4, sd_4, sd_4, sd_4,
                                          sd_5, sd_5, sd_5, sd_5
                                          ])
            model.load_weights(work_path + "weights/A_resnet50_weights_tf_dim_ordering_tf_kernels_notop.h5",
                               by_name=True)
            model.load_weights(work_path + "weights/M_resnet50_weights_tf_dim_ordering_tf_kernels_notop.h5",
                               by_name=True)
        elif phase == 'vis':
            pred = Conv2D(1, (1, 1), padding='same', activation='sigmoid')(A_conv_5_out_M2)
            model = Model(inputs=input_tensors, outputs=[pred, sd_3, sd_4, sd_5,
                                                         m3, m4, m5])
...
def TD_model_prior_masks(input_tensors=None, f1_train=True, stateful=False):
    ...
    if phase == 'train':
        ...
        model = Model(inputs=input_tensors,
                      outputs=[outs, outs, outs, outs,
                               aux_out1, aux_out1, aux_out1, aux_out1,
                               aux_out2, aux_out2, aux_out2, aux_out2,
                               aux_out3, aux_out3, aux_out1, aux_out3
                               ],
                      name='TD_model_prior')
    elif phase == 'vis':
        # for visualization
...
if __name__ == '__main__':
    if phase == 'train':
        ...
        if model_no == 0:
            ximgs_shape = (shape_r, shape_c, 3 + 2 * opt_num)
            input_tensors = Input(shape=ximgs_shape)

            m = Feature_dcross_res_matt_res_ds_masks(input_tensors)
            save_name = 'vap_model/U_td3_res_ds.{epoch:02d}-{val_loss:.4f}.h5'

            m.compile(Adam(lr=1e-5),
                      loss=[kl_divergence, correlation_coefficient, nss, sim,
                            kl_divergence, correlation_coefficient, nss, sim,
                            kl_divergence, correlation_coefficient, nss, sim,
                            kl_divergence, correlation_coefficient, nss, sim
                            ],
                      loss_weights=[kl_weight, cc_weight, nss_weight, sim_weight,
                                    kl_weight, cc_weight, nss_weight, sim_weight,
                                    kl_weight, cc_weight, nss_weight, sim_weight,
                                    kl_weight, cc_weight, nss_weight, sim_weight
                                    ]
                      )
            print("Training diem feature for video")

            m.fit_generator(generator_td_simple_mopt(phase_gen='train'), nb_train // video_b_s, epochs=nb_epoch,
                            initial_epoch=0,  # 11 for 12/N, weight11
                            validation_data=generator_td_simple_mopt(phase_gen='test'),
            ...
            print("Training attendGRU for video")

            m.fit_generator(generator_td_prior_mopt_ds(phase_gen='train'), nb_train // video_b_s, epochs=nb_epoch,
                            initial_epoch=0,
                            validation_data=generator_td_prior_mopt_ds(phase_gen='test'),
                            validation_steps=nb_videos_val // video_b_s,
                            callbacks=[EarlyStopping(patience=60),
                                       ModelCheckpoint(
                                           model_path + save_name,
        ...
        if model_no == 0:
            ...
            # m=Feature_resnet_matt_ds2(input_tensors)
            m = Feature_dcross_res_matt_res_ds_masks(input_tensors)

            weight_name = 'U_td3_res_ds'
            print("Loading %s.h5" % weight_name)
            m.load_weights(model_path + 'vap_model/%s.h5' % weight_name)

            output = work_path + 'vap_predictions/' + DataSets[0] + '/%s/' % weight_name  # before sigma==7

            videos_test_paths = videos_val_paths
            videos = [videos_val_path + f for videos_val_path in videos_test_paths for f in
            ...
            for i in range(len(videos)):
                ...
                prediction = m.predict_generator(get_test_td_simple_mopt(videos[i]),
